...
module.exports.getAllGoods = function(params,cb) {
	...
	conditions["columns"] = params["columns"] ? params["columns"] : {};
	if(params.query) {
	...
	dao.countByConditions("GoodModel",conditions,function(err,count){
		...
		conditions["only"] = ["goods_id","cat_id","goods_name","goods_price","goods_weight","goods_state","add_time","goods_number","upd_time","hot_mumber","is_promote","cat_one_id","cat_two_id","cat_three_id","goods_big_logo","goods_small_logo"];
		...
		dao.list("GoodModel",conditions,function(err,goods){
			...
			resultDta["goods"] = _.map(goods,function(good){
				return _.omit(good,"goods_introduce","is_del","delete_time","goods_state");
			});
			cb(err,resultDta);
		})
	});
 ...
 * @param {*} cb 
 */
module.exports.getGoodListByIds = function(goods_ids,cb){
	/**
	var conditions = {};
	conditions["only"] = ["goods_id","goods_name","goods_price","goods_small_logo"];
	conditions["columns"] = {};
	conditions["columns"]["goods_id"] = goods_ids.split(',');

	dao.list("GoodModel",conditions,function(err,goodslist){
		if(err) return cb(err);
		cb(null,goodslist);
	})
	**/
	db = databaseModule.getDatabase();
	sql = `SELECT goods_id,goods_name,goods_price,goods_small_logo FROM sp_goods WHERE goods_id in (${goods_ids})`;
 ...
 * @param {*} cb 
 */
module.exports.qsearch = function(query,cb) {
	if(!query || query == "") return cb("查询字符串为空");
	var conditions = {};
	conditions["offset"] = 0;
	conditions["limit"] = 20;
	conditions["only"] = ["goods_id","goods_name"];
	conditions["order"] = "-add_time";
	conditions["columns"] = {};
	...
	dao.list("GoodModel",conditions,function(err,goods){
		if(err) return cb(err);
		cb(null,_.map(goods,function(good){
			return _.omit(good,"cat_id","add_time","cat_one_id","cat_two_id","cat_three_id","goods_introduce","is_del","goods_big_logo","goods_small_logo","delete_time","goods_number","goods_price","goods_state","goods_weight","goods_weight","hot_mumber","is_promote","upd_time");
		}));
	})
}
 ...
 * @param {*} cb 
 */
module.exports.randGoodsList = function(size,cb){
	db = databaseModule.getDatabase();
	sql = "SELECT goods_id,goods_name,goods_price,goods_big_logo,goods_small_logo,cat_id FROM sp_goods WHERE goods_big_logo != '' ORDER BY  RAND() LIMIT ?";
